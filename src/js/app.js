App = {
 ...
 timeDiff: function ( tstart, tend ) {
    const timeDifference = tend - tstart;
  ...
  render: function() {
    ...
    App.contracts.Election.deployed().then(function(instance) {
      ...
      loader.hide();
      content.show();
      return electionInstance.deadline();
    }).catch(function(error) {
      console.warn(error);
    }).then(function(deadline) {
      time_left=App.timeDiff(Math.floor(Date.now() / 1000), deadline);
      
      if (deadline.toNumber()<=Math.floor(Date.now() / 1000)) {
        $('form').hide();
        $('#timeLeft').hide();
      }
      $('#timeLeft').html(("Time Left: "+time_left).toString());
    });
  },
